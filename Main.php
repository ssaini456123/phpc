...
class Preprocessor
{
    ...
    public function __construct($src)
    {
        ...
        $this->pos = 0;
        $this->def_table = array();
    }

    public function next_slot_open($tokens, $pos)
    {
        $next = $pos + 1;
        $len = count($tokens);
        if ($next > $len) {
    ...
    private function handle_local_include_directive($resolved_path)
    {
        echo($resolved_path . PHP_EOL);   
    }

    private function handle_ifdef_directive()
    {

    }

    private function handle_endif_directive()
    {
    ...
    private function parse_directive() 
    {
        ...
        switch($directive_name)
        {
            case 'include':
                {
                    ...
                    $is_local_include = $this->tokens[$next]->get_type() === TokenType::STRING_LIT;
                    
                    $is_clib_include = null; // we have to go forward to verify

                    if($is_local_include)
                    {
                        $name = $this->tokens[$next]->get_text();
                        $trimmed_name = trim($name,'"');
                        $this->handle_local_include_directive($trimmed_name);
                    } else {
                        if($this->tokens[$next]->get_type() === TokenType::OPEN_ANGLE_BRACKET)
                        {
                            $next++;
                            $include_path = '';

                            // the series of idents is the include path
                            $n_tok=$this->tokens[$next];

                            while($n_tok->get_type() == TokenType::IDENT || $n_tok->get_type() == TokenType::DOT)
                            {
                                $include_path .= $n_tok->get_text();
                                $next++;
                                $n_tok = $this->tokens[$next];
                            }

                            echo($include_path . PHP_EOL);
                            $is_clib_include = true;
                        }
                        
                        if($is_clib_include) echo("TODO: The cstdlib include files, we might need to make fake ones");
                    }
                    break;
                }
            case 'ifdef':
            ...
            case 'endif':
                ...
                break;
            case 'if':
                echo("hit the stupid fucking if directive" . PHP_EOL);
                break;
            case 'elif':
                echo('hit the stupid fucking elif directive' . PHP_EOL);
                break;
            case 'ifndef':
                echo("hit the endif directive" . PHP_EOL);
                break;
            case 'undef':
                echo("hit the undef macro" . PHP_EOL);
                break;
        }
    }
